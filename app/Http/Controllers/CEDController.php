...
class CEDController extends Controller
{
    public function ced()
    {
        ...
        $activeRequest = DB::select('SELECT * FROM request WHERE department = "CED" AND request_status != 4 AND request_status != 5 AND requested_by = ?', [$usersession]);
        $countActive = count($activeRequest);

        $closeRequest = DB::select('SELECT * FROM request WHERE department = "CED" AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        $countClosed = count($closeRequest);

        return view('ced/ced', ['notifications' => $notifications], compact('username', 'itemJson', 'countItem', 'countActive', 'countClosed'));
    }
    ...
    public function cedproplan()
    {
        ...
        return view('ced/cedproplan', ['proplandata' => $data], ['notifications' => $notifications]);
    }
    ...
    public function requestlist()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "CED" AND request_status != 4 AND request_status != 5 AND deleted_at IS NULL AND requested_by = ?', [$usersession]);
        ...
        return view('ced/cedrequest', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function requestlist2()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "CED" AND deleted_at IS NULL AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        ...
        return view('ced/cedrequest2', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    ...
    public function cedaddrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            foreach ($quantity as $index => $data) {
                ...
                $insert->ced_preriv_id = $request_id;
            ...
            $count_data = CED_Order_List_Model::where('ced_preriv_id', $request_id)->count();

            $total_data = CED_Order_List_Model::where('ced_preriv_id', $request_id)->value(DB::raw("SUM(quantity * unit_price)"));
            ...
            try {
                ...
                return redirect('cedrequest')->with('message', 'failed');
            }

            return redirect('cedrequest')->with('message', 'success');
        }
    }

    public function ceddetails(Request $request)
    {
        ...
        if ($approve_order == NULL) {
            ...
            foreach ($approve_order as  $itemcheck) {
                ...
                $approve_item[] = DB::select('SELECT ItemDet FROM cedppmp WHERE ppmpID = ?', [$item]);
            }
        }

        return view('ced/cedhistory', ['requesthistory' => $req_data], compact('order_data', 'item_data', 'approve_order', 'approve_item', 'user_data', 'requestor_data', 'notifications'));
    }

    public function ceddetails2(Request $request)
    {
    ...
    public function cedrequestview(Request $request)
    {
        ...
        return view('ced/cedrequestview', ['requestview' => $req_data], compact('order_data', 'item_data', 'requestor_data', 'notifications'));
    }

    public function cedrequestedit(Request $request)
    {
        ...
        return view('ced/cedrequestedit', ['requestview' => $req_data], compact('order_data', 'item_data', 'notifications'));
    }

    public function cededitrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            return redirect('cedrequest')->with('message', 'update');
        }
    }
    ...
    public function deleterequest(Request $request)
    {
        ...
        return redirect('cedrequest');
    }

    public function cedtrash()
    {
        $usersession = Session::get('user');
        $user = AdminModel::find($usersession);
        $notifications = $user->unreadNotifications;

        $data = DB::select('SELECT * FROM request WHERE department = "CED" AND deleted_at IS NOT NULL AND requested_by = ?', [$usersession]);
        ...
        return view('ced/cedtrash', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function cedrestoreall()
    {
        $restore = RequestModel::onlyTrashed()->restore();

        return redirect('cedtrash')->with('message', 'restored');;
    }
    public function cederaseall()
    {
        $restore = RequestModel::onlyTrashed()->forceDelete();

        return redirect('cedtrash')->with('message', 'erased');;
    }
    public function cedrestore(Request $request)
    {
    ...
    public function cederase(Request $request)
    {
