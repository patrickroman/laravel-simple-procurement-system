...
class SGOController extends Controller
{
    public function sgo()
    {
        ...
        $activeRequest = DB::select('SELECT * FROM request WHERE department = "SGO" AND request_status != 4 AND request_status != 5 AND requested_by = ?', [$usersession]);
        $countActive = count($activeRequest);

        $closeRequest = DB::select('SELECT * FROM request WHERE department = "SGO" AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        $countClosed = count($closeRequest);

        return view('sgo/sgo', ['notifications' => $notifications], compact('username', 'itemJson', 'countItem', 'countActive', 'countClosed'));
    }
    ...
    public function sgoproplan()
    {
        ...
        return view('sgo/sgoproplan', ['proplandata' => $data], ['notifications' => $notifications]);
    }
    ...
    public function requestlist()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "SGO" AND request_status != 4 AND request_status != 5 AND deleted_at IS NULL AND requested_by = ?', [$usersession]);
        ...
        return view('sgo/sgorequest', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function requestlist2()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "SGO" AND deleted_at IS NULL AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        ...
        return view('sgo/sgorequest2', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    ...
    public function sgoaddrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            if($quantity == NULL){
                ...
                return redirect('sgorequestadd')->with('message', 'error');
            }

            foreach ($quantity as $index => $data) {
                $insert = new SGO_Order_List_Model;

                $insert->sgo_preriv_id = $request_id;
            ...
            $count_data = SGO_Order_List_Model::where('sgo_preriv_id', $request_id)->count();

            $total_data = SGO_Order_List_Model::where('sgo_preriv_id', $request_id)->value(DB::raw("SUM(quantity * unit_price)"));
            ...
            try {
                ...
                return redirect('sgorequest')->with('message', 'failed');
            }

            return redirect('sgorequest')->with('message', 'success');
        }
    }

    public function sgodetails(Request $request)
    {
        ...
        if ($approve_order == NULL) {
            ...
            foreach ($approve_order as  $itemcheck) {
                ...
                $approve_item[] = DB::select('SELECT ItemDet FROM sgoppmp WHERE ppmpID = ?', [$item]);
            }
        }
        return view('sgo/sgohistory', ['requesthistory' => $req_data], compact('order_data', 'item_data', 'approve_order', 'approve_item', 'user_data', 'requestor_data', 'notifications'));
    }

    public function sgodetails2(Request $request)
    {
    ...
    public function sgorequestview(Request $request)
    {
        ...
        return view('sgo/sgorequestview', ['requestview' => $req_data], compact('order_data', 'item_data', 'requestor_data', 'notifications'));
    }

    public function sgorequestedit(Request $request)
    {
        ...
        return view('sgo/sgorequestedit', ['requestview' => $req_data], compact('order_data', 'item_data', 'notifications'));
    }

    public function sgoeditrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            return redirect('sgorequest')->with('message', 'update');
        }
    }
    ...
    public function deleterequest(Request $request)
    {
        ...
        return redirect('sgorequest');
    }

    public function sgotrash()
    {
        $usersession = Session::get('user');
        $user = AdminModel::find($usersession);
        $notifications = $user->unreadNotifications;

        $data = DB::select('SELECT * FROM request WHERE department = "SGO" AND deleted_at IS NOT NULL AND requested_by = ?', [$usersession]);
        ...
        return view('sgo/sgotrash', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function sgorestoreall()
    {
        $restore = RequestModel::onlyTrashed()->restore();

        return redirect('sgotrash')->with('message', 'restored');
    }
    public function sgoeraseall()
    {
        $restore = RequestModel::onlyTrashed()->forceDelete();

        return redirect('sgotrash')->with('message', 'erased');
    }
    public function sgorestore(Request $request)
    {
    ...
    public function sgoerase(Request $request)
    {
