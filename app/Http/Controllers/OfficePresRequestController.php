...
class OfficePresRequestController extends Controller
{
    public function presidentdashboard()
    {
        ...
        $activeRequest = DB::select('SELECT * FROM request WHERE department = "President" AND request_status != 4 AND request_status != 5 AND requested_by = ?', [$usersession]);
        $countActive = count($activeRequest);

        $closeRequest = DB::select('SELECT * FROM request WHERE department = "President" AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        $countClosed = count($closeRequest);

        return view('president staff/president', ['notifications' => $notifications], compact('username', 'itemJson', 'countItem', 'countActive', 'countClosed'));
    }
    ...
    public function presidentproplan()
    {
        ...
        return view('president staff/presidentproplan', ['proplandata' => $data], ['notifications' => $notifications]);
    }
    ...
    public function requestlist()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "President" AND request_status != 4 AND request_status != 5 AND deleted_at IS NULL AND requested_by = ?', [$usersession]);
        ...
        return view('president staff/presidentrequest', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function requestlist2()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "President" AND deleted_at IS NULL AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        ...
        return view('president staff/presidentrequest2', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    ...
    public function presidentaddrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            foreach ($quantity as $index => $data) {
                ...
                $insert->president_preriv_id = $request_id;
            ...
            $count_data = President_Order_List_Model::where('president_preriv_id', $request_id)->count();

            $total_data = President_Order_List_Model::where('president_preriv_id', $request_id)->value(DB::raw("SUM(quantity * unit_price)"));
            ...
            return redirect('presidentrequest')->with('message', 'success');
        }
    }

    public function presidentdetails(Request $request)
    {
        ...
        if ($approve_order == NULL) {
            ...
            foreach ($approve_order as  $itemcheck) {
                ...
                $approve_item[] = DB::select('SELECT ItemDet FROM president WHERE ppmpID = ?', [$item]);
            }
        }

        return view('president staff/presidenthistory', ['requesthistory' => $req_data], compact('order_data', 'item_data', 'approve_order', 'approve_item', 'user_data', 'requestor_data', 'notifications'));
    }

    public function presidentdetails2(Request $request)
    {
    ...
    public function presidentrequestview(Request $request)
    {
        ...
        return view('president staff/presidentrequestview', ['requestview' => $req_data], compact('order_data', 'item_data', 'requestor_data', 'notifications'));
    }

    public function presidentrequestedit(Request $request)
    {
        ...
        return view('president staff/presidentrequestedit', ['requestview' => $req_data], compact('order_data', 'item_data', 'notifications'));
    }

    public function presidenteditrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            return redirect('presidentrequest')->with('message', 'update');
        }
    }
    ...
    public function deleterequest(Request $request)
    {
        ...
        return redirect('presidentrequest');
    }

    public function presidenttrash()
    {
        $usersession = Session::get('user');
        $user = AdminModel::find($usersession);
        $notifications = $user->unreadNotifications;

        $data = DB::select('SELECT * FROM request WHERE department = "President" AND deleted_at IS NOT NULL AND requested_by = ?', [$usersession]);
        ...
        return view('president/presidenttrash', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function presidentrestoreall()
    {
        $restore = RequestModel::onlyTrashed()->restore();

        return redirect('presidenttrash')->with('message', 'restored');;
    }
    public function presidenteraseall()
    {
        $restore = RequestModel::onlyTrashed()->forceDelete();

        return redirect('presidenttrash')->with('message', 'erased');;
    }
    public function presidentrestore(Request $request)
    {
    ...
    public function presidenterase(Request $request)
    {
