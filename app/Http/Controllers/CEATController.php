...
class CEATController extends Controller
{
    public function ceat()
    {
        ...
        $activeRequest = DB::select('SELECT * FROM request WHERE department = "CEAT" AND request_status != 4 AND request_status != 5 AND requested_by = ?', [$usersession]);
        $countActive = count($activeRequest);

        $closeRequest = DB::select('SELECT * FROM request WHERE department = "CEAT" AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        $countClosed = count($closeRequest);

        return view('ceat/ceat', ['notifications' => $notifications], compact('username', 'itemJson', 'countItem', 'countActive', 'countClosed'));
    }
    public function ceatproplan()
    {
        ...
        return view('ceat/ceatproplan', ['proplandata' => $data]);
    }
    ...
    public function requestlist()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "CEAT" AND request_status != 4 AND request_status != 5 AND deleted_at IS NULL AND requested_by = ?', [$usersession]);
        ...
        return view('ceat/ceatrequest', compact('data', 'requestor_data'));
    }
    ...
    public function requestlist2()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "CEAT" AND deleted_at IS NULL AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        ...
        return view('ceat/ceatrequest2', compact('data', 'requestor_data'));
    }
    ...
    public function ceataddrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            foreach ($quantity as $index => $data) {
                ...
                $insert->ceat_preriv_id = $request_id;
            ...
            $count_data = CEAT_Order_List_Model::where('ceat_preriv_id', $request_id)->count();

            $total_data = CEAT_Order_List_Model::where('ceat_preriv_id', $request_id)->value(DB::raw("SUM(quantity * unit_price)"));
            ...
            return redirect('ceatrequest');
        }
    }

    public function ceatdetails(Request $request)
    {
        ...
        if ($approve_order == NULL) {
            ...
            foreach ($approve_order as  $itemcheck) {
                ...
                $approve_item[] = DB::select('SELECT ItemDet FROM ceatppmp WHERE ppmpID = ?', [$item]);
            }
        }

        return view('ceat/ceathistory', ['requesthistory' => $req_data], compact('order_data', 'item_data', 'approve_order', 'approve_item', 'user_data', 'requestor_data'));
    }

    public function ceatdetails2(Request $request)
    {
        ...
        return view('ceat/ceathistory2', ['requesthistory' => $req_data], compact('order_data', 'item_data', 'approve_order', 'approve_item', 'user_data', 'requestor_data'));
    }

    public function ceatrequestview(Request $request)
    {
        ...
        return view('ceat/ceatrequestview', ['requestview' => $req_data], compact('order_data', 'item_data', 'requestor_data'));
    }

    public function ceatrequestedit(Request $request)
    {
        ...
        return view('ceat/ceatrequestedit', ['requestview' => $req_data], compact('order_data', 'item_data'));
    }

    public function ceateditrequest(Request $request)
    {
    ...
    public function ceatdeleterequest(Request $request)
    {
    ...
    public function ceattrash()
    {
        $usersession = Session::get('user');
        $user = AdminModel::find($usersession);
        $notifications = $user->unreadNotifications;

        $data = DB::select('SELECT * FROM request WHERE department = "CEAT" AND deleted_at IS NOT NULL AND requested_by = ?', [$usersession]);
        ...
        return view('ceat/ceattrash', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function ceatrestoreall()
    {
        $restore = RequestModel::onlyTrashed()->restore();

        return redirect('ceattrash')->with('message', 'restored');;
    }
    public function ceateraseall()
    {
        $restore = RequestModel::onlyTrashed()->forceDelete();

        return redirect('ceattrash')->with('message', 'erased');;
    }
    public function ceatrestore(Request $request)
    {
    ...
    public function ceaterase(Request $request)
    {
