...
class FinanceRequestController extends Controller
{
    public function financedashboard()
    {
        ...
        $activeRequest = DB::select('SELECT * FROM request WHERE department = "Finance" AND request_status != 4 AND request_status != 5 AND requested_by = ?', [$usersession]);
        $countActive = count($activeRequest);

        $closeRequest = DB::select('SELECT * FROM request WHERE department = "Finance" AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        $countClosed = count($closeRequest);

        return view('finance staff/finance', ['notifications' => $notifications], compact('username', 'itemJson', 'countItem', 'countActive', 'countClosed'));
    }
    ...
    public function financeproplan()
    {
        ...
        return view('finance staff/financeproplan', ['proplandata' => $data], ['notifications' => $notifications]);
    }
    ...
    public function requestlist()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "Finance" AND request_status != 4 AND request_status != 5 AND deleted_at IS NULL AND requested_by = ?', [$usersession]);
        ...
        return view('finance staff/financerequest', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function requestlist2()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "Finance" AND deleted_at IS NULL AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        ...
        return view('finance staff/financerequest2', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    ...
    public function financeaddrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            foreach ($quantity as $index => $data) {
                ...
                $insert->finance_preriv_id = $request_id;
            ...
            $count_data = Finance_Order_List_Model::where('finance_preriv_id', $request_id)->count();

            $total_data = Finance_Order_List_Model::where('finance_preriv_id', $request_id)->value(DB::raw("SUM(quantity * unit_price)"));
            ...
            return redirect('financerequest')->with('message', 'success');
        }
    }
    ...
    public function financedetails(Request $request)
    {
        ...
        if ($approve_order == NULL) {
            ...
            foreach ($approve_order as  $itemcheck) {
                ...
                $approve_item[] = DB::select('SELECT ItemDet FROM finance WHERE ppmpID = ?', [$item]);
            }
        }

        return view('finance staff/financehistory', ['requesthistory' => $req_data], compact('order_data', 'item_data', 'approve_order', 'approve_item', 'user_data', 'requestor_data', 'notifications'));
    }

    public function financedetails2(Request $request)
    {
    ...
    public function financerequestview(Request $request)
    {
        ...
        return view('finance staff/financerequestview', ['requestview' => $req_data], compact('order_data', 'item_data', 'requestor_data', 'notifications'));
    }

    public function financerequestedit(Request $request)
    {
        ...
        return view('finance staff/financerequestedit', ['requestview' => $req_data], compact('order_data', 'item_data', 'notifications'));
    }

    public function financeeditrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            $count_data = Finance_Order_List_Model::where('finance_preriv_id', $req_id)->count();

            $total_data = Finance_Order_List_Model::where('finance_preriv_id', $req_id)->value(DB::raw("SUM(quantity * unit_price)"));
            ...
            return redirect('financerequest')->with('message', 'update');
        }
    }
    ...
    public function financetrash()
    {
        $usersession = Session::get('user');
        $user = AdminModel::find($usersession);
        $notifications = $user->unreadNotifications;

        $data = DB::select('SELECT * FROM request WHERE department = "Finance" AND deleted_at IS NOT NULL AND requested_by = ?', [$usersession]);
        ...
        return view('finance/financetrash', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function financerestoreall()
    {
        $restore = RequestModel::onlyTrashed()->restore();

        return redirect('financetrash')->with('message', 'restored');;
    }
    public function financeeraseall()
    {
        $restore = RequestModel::onlyTrashed()->forceDelete();

        return redirect('financetrash')->with('message', 'erased');;
    }
    public function financerestore(Request $request)
    {
    ...
    public function financeerase(Request $request)
    {
