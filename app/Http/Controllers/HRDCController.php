...
class HRDCController extends Controller
{
    public function hrdc()
    {
        ...
        $activeRequest = DB::select('SELECT * FROM request WHERE department = "HRDC" AND request_status != 4 AND request_status != 5 AND requested_by = ?', [$usersession]);
        $countActive = count($activeRequest);

        $closeRequest = DB::select('SELECT * FROM request WHERE department = "HRDC" AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        $countClosed = count($closeRequest);

        return view('hrdc/hrdc', ['notifications' => $notifications], compact('username', 'itemJson', 'countItem', 'countActive', 'countClosed'));
    }
    ...
    public function hrdcproplan()
    {
        ...
        return view('hrdc/hrdcproplan', ['proplandata' => $data], ['notifications' => $notifications]);
    }
    ...
    public function requestlist()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "HRDC" AND request_status != 4 AND request_status != 5 AND deleted_at IS NULL AND requested_by = ?', [$usersession]);
        ...
        return view('hrdc/hrdcrequest', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function requestlist2()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "HRDC" AND deleted_at IS NULL AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        ...
        return view('hrdc/hrdcrequest2', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    ...
    public function hrdcaddrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            if($quantity == NULL){
                ...
                return redirect('hrdcrequestadd')->with('message', 'error');
            }

            foreach ($quantity as $index => $data) {
                $insert = new HRDC_Order_List_Model;

                $insert->hrdc_preriv_id = $request_id;
            ...
            $count_data = HRDC_Order_List_Model::where('hrdc_preriv_id', $request_id)->count();

            $total_data = HRDC_Order_List_Model::where('hrdc_preriv_id', $request_id)->value(DB::raw("SUM(quantity * unit_price)"));
            ...
            try {
                ...
                return redirect('hrdcrequest')->with('message', 'failed');
            }

            return redirect('hrdcrequest')->with('message', 'success');
        }
    }

    public function hrdcdetails(Request $request)
    {
        ...
        if ($approve_order == NULL) {
            ...
            foreach ($approve_order as  $itemcheck) {
                ...
                $approve_item[] = DB::select('SELECT ItemDet FROM hrdcppmp WHERE ppmpID = ?', [$item]);
            }
        }
        return view('hrdc/hrdchistory', ['requesthistory' => $req_data], compact('order_data', 'item_data', 'approve_order', 'approve_item', 'user_data', 'requestor_data', 'notifications'));
    }

    public function hrdcdetails2(Request $request)
    {
    ...
    public function hrdcrequestview(Request $request)
    {
        ...
        return view('hrdc/hrdcrequestview', ['requestview' => $req_data], compact('order_data', 'item_data', 'requestor_data', 'notifications'));
    }

    public function hrdcrequestedit(Request $request)
    {
        ...
        return view('hrdc/hrdcrequestedit', ['requestview' => $req_data], compact('order_data', 'item_data', 'notifications'));
    }

    public function hrdceditrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            return redirect('hrdcrequest')->with('message', 'update');
        }
    }
    ...
    public function deleterequest(Request $request)
    {
        ...
        return redirect('hrdcrequest');
    }

    public function hrdctrash()
    {
        $usersession = Session::get('user');
        $user = AdminModel::find($usersession);
        $notifications = $user->unreadNotifications;

        $data = DB::select('SELECT * FROM request WHERE department = "HRDC" AND deleted_at IS NOT NULL AND requested_by = ?', [$usersession]);
        ...
        return view('hrdc/hrdctrash', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function hrdcrestoreall()
    {
        $restore = RequestModel::onlyTrashed()->restore();

        return redirect('hrdctrash')->with('message', 'restored');
    }
    public function hrdceraseall()
    {
        $restore = RequestModel::onlyTrashed()->forceDelete();

        return redirect('hrdctrash')->with('message', 'erased');
    }
    public function hrdcrestore(Request $request)
    {
    ...
    public function hrdcerase(Request $request)
    {
