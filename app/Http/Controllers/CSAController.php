...
class CSAController extends Controller
{
    public function csa()
    {
        ...
        $activeRequest = DB::select('SELECT * FROM request WHERE department = "CSA" AND request_status != 4 AND request_status != 5 AND requested_by = ?', [$usersession]);
        $countActive = count($activeRequest);

        $closeRequest = DB::select('SELECT * FROM request WHERE department = "CSA" AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        $countClosed = count($closeRequest);

        return view('csa/csa', ['notifications' => $notifications], compact('username', 'itemJson', 'countItem', 'countActive', 'countClosed'));
    }
    ...
    public function csaproplan()
    {
        ...
        return view('csa/csaproplan', ['proplandata' => $data], ['notifications' => $notifications]);
    }
    ...
    public function requestlist()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "CSA" AND request_status != 4 AND request_status != 5 AND deleted_at IS NULL AND requested_by = ?', [$usersession]);
        ...
        return view('csa/csarequest', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function requestlist2()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "CSA" AND deleted_at IS NULL AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        ...
        return view('csa/csarequest2', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    ...
    public function csaaddrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            if($quantity == NULL){
                ...
                return redirect('csarequestadd')->with('message', 'error');
            }

            foreach ($quantity as $index => $data) {
                $insert = new CSA_Order_List_Model;

                $insert->csa_preriv_id = $request_id;
            ...
            $count_data = CSA_Order_List_Model::where('csa_preriv_id', $request_id)->count();

            $total_data = CSA_Order_List_Model::where('csa_preriv_id', $request_id)->value(DB::raw("SUM(quantity * unit_price)"));
            ...
            try {
                ...
                return redirect('csarequest')->with('message', 'failed');
            }

            return redirect('csarequest')->with('message', 'success');
        }
    }

    public function csadetails(Request $request)
    {
        ...
        if ($approve_order == NULL) {
            ...
            foreach ($approve_order as  $itemcheck) {
                ...
                $approve_item[] = DB::select('SELECT ItemDet FROM csappmp WHERE ppmpID = ?', [$item]);
            }
        }
        return view('csa/csahistory', ['requesthistory' => $req_data], compact('order_data', 'item_data', 'approve_order', 'approve_item', 'user_data', 'requestor_data', 'notifications'));
    }

    public function csadetails2(Request $request)
    {
    ...
    public function csarequestview(Request $request)
    {
        ...
        return view('csa/csarequestview', ['requestview' => $req_data], compact('order_data', 'item_data', 'requestor_data', 'notifications'));
    }

    public function csarequestedit(Request $request)
    {
        ...
        return view('csa/csarequestedit', ['requestview' => $req_data], compact('order_data', 'item_data', 'notifications'));
    }

    public function csaeditrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            return redirect('csarequest')->with('message', 'update');
        }
    }
    ...
    public function deleterequest(Request $request)
    {
        ...
        return redirect('csarequest');
    }

    public function csatrash()
    {
        $usersession = Session::get('user');
        $user = AdminModel::find($usersession);
        $notifications = $user->unreadNotifications;

        $data = DB::select('SELECT * FROM request WHERE department = "CSA" AND deleted_at IS NOT NULL AND requested_by = ?', [$usersession]);
        ...
        return view('csa/csatrash', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function csarestoreall()
    {
        $restore = RequestModel::onlyTrashed()->restore();

        return redirect('csatrash')->with('message', 'restored');
    }
    public function csaeraseall()
    {
        $restore = RequestModel::onlyTrashed()->forceDelete();

        return redirect('csatrash')->with('message', 'erased');
    }
    public function csarestore(Request $request)
    {
    ...
    public function csaerase(Request $request)
    {
