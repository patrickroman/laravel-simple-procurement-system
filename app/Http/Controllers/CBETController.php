...
class CBETController extends Controller
{
    public function cbet()
    {
        ...
        $activeRequest = DB::select('SELECT * FROM request WHERE department = "CBET" AND request_status != 4 AND request_status != 5 AND requested_by = ?', [$usersession]);
        $countActive = count($activeRequest);

        $closeRequest = DB::select('SELECT * FROM request WHERE department = "CBET" AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        $countClosed = count($closeRequest);

        return view('cbet/cbet', ['notifications' => $notifications], compact('username', 'itemJson', 'countItem', 'countActive', 'countClosed'));
    }
    ...
    public function cbetproplan()
    {
        ...
        return view('cbet/cbetproplan', ['proplandata' => $data], ['notifications' => $notifications]);
    }
    ...
    public function requestlist()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "CBET" AND request_status != 4 AND request_status != 5 AND deleted_at IS NULL AND requested_by = ?', [$usersession]);
        ...
        return view('cbet/cbetrequest', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function requestlist2()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "CBET" AND deleted_at IS NULL AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        ...
        return view('cbet/cbetrequest2', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    ...
    public function cbetaddrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            foreach ($quantity as $index => $data) {
                ...
                $insert->cbet_preriv_id = $request_id;
            ...
            $count_data = CBET_Order_List_Model::where('cbet_preriv_id', $request_id)->count();

            $total_data = CBET_Order_List_Model::where('cbet_preriv_id', $request_id)->value(DB::raw("SUM(quantity * unit_price)"));
            ...
            try {
                ...
                return redirect('cbetrequest')->with('message', 'failed');
            }

            return redirect('cbetrequest')->with('message', 'success');
        }
    }

    public function cbetdetails(Request $request)
    {
        ...
        if ($approve_order == NULL) {
            ...
            foreach ($approve_order as  $itemcheck) {
                ...
                $approve_item[] = DB::select('SELECT ItemDet FROM cbetppmp WHERE ppmpID = ?', [$item]);
            }
        }

        return view('cbet/cbethistory', ['requesthistory' => $req_data], compact('order_data', 'item_data', 'approve_order', 'approve_item', 'user_data', 'requestor_data', 'notifications'));
    }

    public function cbetdetails2(Request $request)
    {
    ...
    public function cbetrequestview(Request $request)
    {
        ...
        return view('cbet/cbetrequestview', ['requestview' => $req_data], compact('order_data', 'item_data', 'requestor_data', 'notifications'));
    }

    public function cbetrequestedit(Request $request)
    {
        ...
        return view('cbet/cbetrequestedit', ['requestview' => $req_data], compact('order_data', 'item_data', 'notifications'));
    }

    public function cbeteditrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            return redirect('cbetrequest')->with('message', 'update');
        }
    }
    ...
    public function deleterequest(Request $request)
    {
        ...
        return redirect('cbetrequest');
    }

    public function cbettrash()
    {
        $usersession = Session::get('user');
        $user = AdminModel::find($usersession);
        $notifications = $user->unreadNotifications;

        $data = DB::select('SELECT * FROM request WHERE department = "CBET" AND deleted_at IS NOT NULL AND requested_by = ?', [$usersession]);
        ...
        return view('cbet/cbettrash', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function cbetrestoreall()
    {
        $restore = RequestModel::onlyTrashed()->restore();

        return redirect('cbettrash')->with('message', 'restored');;
    }
    public function cbeteraseall()
    {
        $restore = RequestModel::onlyTrashed()->forceDelete();

        return redirect('cbettrash')->with('message', 'erased');;
    }
    public function cbetrestore(Request $request)
    {
    ...
    public function cbeterase(Request $request)
    {
