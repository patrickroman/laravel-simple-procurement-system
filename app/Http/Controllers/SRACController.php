...
class SRACController extends Controller
{
    public function srac()
    {
        ...
        $activeRequest = DB::select('SELECT * FROM request WHERE department = "SRAC" AND request_status != 4 AND request_status != 5 AND requested_by = ?', [$usersession]);
        $countActive = count($activeRequest);

        $closeRequest = DB::select('SELECT * FROM request WHERE department = "SRAC" AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        $countClosed = count($closeRequest);

        return view('srac/srac', ['notifications' => $notifications], compact('username', 'itemJson', 'countItem', 'countActive', 'countClosed'));
    }
    ...
    public function sracproplan()
    {
        ...
        return view('srac/sracproplan', ['proplandata' => $data], ['notifications' => $notifications]);
    }
    ...
    public function requestlist()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "SRAC" AND request_status != 4 AND request_status != 5 AND deleted_at IS NULL AND requested_by = ?', [$usersession]);
        ...
        return view('srac/sracrequest', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function requestlist2()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "SRAC" AND deleted_at IS NULL AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        ...
        return view('srac/sracrequest2', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    ...
    public function sracaddrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            if($quantity == NULL){
                ...
                return redirect('sracrequestadd')->with('message', 'error');
            }

            foreach ($quantity as $index => $data) {
                $insert = new SRAC_Order_List_Model;

                $insert->srac_preriv_id = $request_id;
            ...
            $count_data = SRAC_Order_List_Model::where('srac_preriv_id', $request_id)->count();

            $total_data = SRAC_Order_List_Model::where('srac_preriv_id', $request_id)->value(DB::raw("SUM(quantity * unit_price)"));
            ...
            try {
                ...
                return redirect('sracrequest')->with('message', 'failed');
            }

            return redirect('sracrequest')->with('message', 'success');
        }
    }

    public function sracdetails(Request $request)
    {
        ...
        if ($approve_order == NULL) {
            ...
            foreach ($approve_order as  $itemcheck) {
                ...
                $approve_item[] = DB::select('SELECT ItemDet FROM sracppmp WHERE ppmpID = ?', [$item]);
            }
        }
        return view('srac/srachistory', ['requesthistory' => $req_data], compact('order_data', 'item_data', 'approve_order', 'approve_item', 'user_data', 'requestor_data', 'notifications'));
    }

    public function sracdetails2(Request $request)
    {
    ...
    public function sracrequestview(Request $request)
    {
        ...
        return view('srac/sracrequestview', ['requestview' => $req_data], compact('order_data', 'item_data', 'requestor_data', 'notifications'));
    }

    public function sracrequestedit(Request $request)
    {
        ...
        return view('srac/sracrequestedit', ['requestview' => $req_data], compact('order_data', 'item_data', 'notifications'));
    }

    public function sraceditrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            return redirect('sracrequest')->with('message', 'update');
        }
    }
    ...
    public function deleterequest(Request $request)
    {
        ...
        return redirect('sracrequest');
    }

    public function sractrash()
    {
        $usersession = Session::get('user');
        $user = AdminModel::find($usersession);
        $notifications = $user->unreadNotifications;

        $data = DB::select('SELECT * FROM request WHERE department = "SRAC" AND deleted_at IS NOT NULL AND requested_by = ?', [$usersession]);
        ...
        return view('srac/sractrash', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function sracrestoreall()
    {
        $restore = RequestModel::onlyTrashed()->restore();

        return redirect('sractrash')->with('message', 'restored');
    }
    public function sraceraseall()
    {
        $restore = RequestModel::onlyTrashed()->forceDelete();

        return redirect('sractrash')->with('message', 'erased');
    }
    public function sracrestore(Request $request)
    {
    ...
    public function sracerase(Request $request)
    {
