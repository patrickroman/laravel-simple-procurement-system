...
class IPEController extends Controller
{
    public function ipe()
    {
        ...
        $activeRequest = DB::select('SELECT * FROM request WHERE department = "IPE" AND request_status != 4 AND request_status != 5 AND requested_by = ?', [$usersession]);
        $countActive = count($activeRequest);

        $closeRequest = DB::select('SELECT * FROM request WHERE department = "IPE" AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        $countClosed = count($closeRequest);

        return view('ipe/ipe', ['notifications' => $notifications], compact('username', 'itemJson', 'countItem', 'countActive', 'countClosed'));
    }
    ...
    public function ipeproplan()
    {
        ...
        return view('ipe/ipeproplan', ['proplandata' => $data], ['notifications' => $notifications]);
    }
    ...
    public function requestlist()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "IPE" AND request_status != 4 AND request_status != 5 AND deleted_at IS NULL AND requested_by = ?', [$usersession]);
        ...
        return view('ipe/iperequest', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function requestlist2()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "IPE" AND deleted_at IS NULL AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        ...
        return view('ipe/iperequest2', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    ...
    public function ipeaddrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            foreach ($quantity as $index => $data) {
                ...
                $insert->ipe_preriv_id = $request_id;
            ...
            $count_data = IPE_Order_List_Model::where('ipe_preriv_id', $request_id)->count();

            $total_data = IPE_Order_List_Model::where('ipe_preriv_id', $request_id)->value(DB::raw("SUM(quantity * unit_price)"));
            ...
            try {
                ...
                return redirect('iperequest')->with('message', 'failed');
            }

            return redirect('iperequest')->with('message', 'success');
        }
    }

    public function ipedetails(Request $request)
    {
        ...
        if ($approve_order == NULL) {
            ...
            foreach ($approve_order as  $itemcheck) {
                ...
                $approve_item[] = DB::select('SELECT ItemDet FROM ipeppmp WHERE ppmpID = ?', [$item]);
            }
        }

        return view('ipe/ipehistory', ['requesthistory' => $req_data], compact('order_data', 'item_data', 'approve_order', 'approve_item', 'user_data', 'requestor_data', 'notifications'));
    }

    public function ipedetails2(Request $request)
    {
    ...
    public function iperequestview(Request $request)
    {
        ...
        return view('ipe/iperequestview', ['requestview' => $req_data], compact('order_data', 'item_data', 'requestor_data', 'notifications'));
    }

    public function iperequestedit(Request $request)
    {
        ...
        return view('ipe/iperequestedit', ['requestview' => $req_data], compact('order_data', 'item_data', 'notifications'));
    }

    public function ipeeditrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            return redirect('iperequest')->with('message', 'update');
        }
    }
    ...
    public function deleterequest(Request $request)
    {
        ...
        return redirect('iperequest');
    }

    public function ipetrash()
    {
        $usersession = Session::get('user');
        $user = AdminModel::find($usersession);
        $notifications = $user->unreadNotifications;

        $data = DB::select('SELECT * FROM request WHERE department = "IPE" AND deleted_at IS NOT NULL AND requested_by = ?', [$usersession]);
        ...
        return view('ipe/ipetrash', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function iperestoreall()
    {
        $restore = RequestModel::onlyTrashed()->restore();

        return redirect('ipetrash')->with('message', 'restored');;
    }
    public function ipeeraseall()
    {
        $restore = RequestModel::onlyTrashed()->forceDelete();

        return redirect('ipetrash')->with('message', 'erased');;
    }
    public function iperestore(Request $request)
    {
    ...
    public function ipeerase(Request $request)
    {
