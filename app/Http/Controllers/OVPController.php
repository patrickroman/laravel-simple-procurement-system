...
class OVPController extends Controller
{
    public function ovp()
    {
        ...
        $activeRequest = DB::select('SELECT * FROM request WHERE department = "OVP" AND request_status != 4 AND request_status != 5 AND requested_by = ?', [$usersession]);
        $countActive = count($activeRequest);

        $closeRequest = DB::select('SELECT * FROM request WHERE department = "OVP" AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        $countClosed = count($closeRequest);

        return view('ovp/ovp', ['notifications' => $notifications], compact('username', 'itemJson', 'countItem', 'countActive', 'countClosed'));
    }
    ...
    public function ovpproplan()
    {
        ...
        return view('ovp/ovpproplan', ['proplandata' => $data], ['notifications' => $notifications]);
    }
    ...
    public function requestlist()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "OVP" AND request_status != 4 AND request_status != 5 AND deleted_at IS NULL AND requested_by = ?', [$usersession]);
        ...
        return view('ovp/ovprequest', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function requestlist2()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "OVP" AND deleted_at IS NULL AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        ...
        return view('ovp/ovprequest2', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    ...
    public function ovpaddrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            if($quantity == NULL){
                ...
                return redirect('ovprequestadd')->with('message', 'error');
            }

            foreach ($quantity as $index => $data) {
                $insert = new OVP_Order_List_Model;

                $insert->ovp_preriv_id = $request_id;
            ...
            $count_data = OVP_Order_List_Model::where('ovp_preriv_id', $request_id)->count();

            $total_data = OVP_Order_List_Model::where('ovp_preriv_id', $request_id)->value(DB::raw("SUM(quantity * unit_price)"));
            ...
            try {
                ...
                return redirect('ovprequest')->with('message', 'failed');
            }

            return redirect('ovprequest')->with('message', 'success');
        }
    }

    public function ovpdetails(Request $request)
    {
        ...
        if ($approve_order == NULL) {
            ...
            foreach ($approve_order as  $itemcheck) {
                ...
                $approve_item[] = DB::select('SELECT ItemDet FROM ovpppmp WHERE ppmpID = ?', [$item]);
            }
        }
        return view('ovp/ovphistory', ['requesthistory' => $req_data], compact('order_data', 'item_data', 'approve_order', 'approve_item', 'user_data', 'requestor_data', 'notifications'));
    }

    public function ovpdetails2(Request $request)
    {
    ...
    public function ovprequestview(Request $request)
    {
        ...
        return view('ovp/ovprequestview', ['requestview' => $req_data], compact('order_data', 'item_data', 'requestor_data', 'notifications'));
    }

    public function ovprequestedit(Request $request)
    {
        ...
        return view('ovp/ovprequestedit', ['requestview' => $req_data], compact('order_data', 'item_data', 'notifications'));
    }

    public function ovpeditrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            return redirect('ovprequest')->with('message', 'update');
        }
    }
    ...
    public function deleterequest(Request $request)
    {
        ...
        return redirect('ovprequest');
    }

    public function ovptrash()
    {
        $usersession = Session::get('user');
        $user = AdminModel::find($usersession);
        $notifications = $user->unreadNotifications;

        $data = DB::select('SELECT * FROM request WHERE department = "OVP" AND deleted_at IS NOT NULL AND requested_by = ?', [$usersession]);
        ...
        return view('ovp/ovptrash', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function ovprestoreall()
    {
        $restore = RequestModel::onlyTrashed()->restore();

        return redirect('ovptrash')->with('message', 'restored');
    }
    public function ovperaseall()
    {
        $restore = RequestModel::onlyTrashed()->forceDelete();

        return redirect('ovptrash')->with('message', 'erased');
    }
    public function ovprestore(Request $request)
    {
    ...
    public function ovperase(Request $request)
    {
