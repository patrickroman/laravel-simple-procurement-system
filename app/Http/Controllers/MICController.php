...
class MICController extends Controller
{
    public function mic()
    {
        ...
        $activeRequest = DB::select('SELECT * FROM request WHERE department = "MIC" AND request_status != 4 AND request_status != 5 AND requested_by = ?', [$usersession]);
        $countActive = count($activeRequest);

        $closeRequest = DB::select('SELECT * FROM request WHERE department = "MIC" AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        $countClosed = count($closeRequest);

        return view('mic/mic', ['notifications' => $notifications], compact('username', 'itemJson', 'countItem', 'countActive', 'countClosed'));
    }
    ...
    public function micproplan()
    {
        ...
        return view('mic/micproplan', ['proplandata' => $data], ['notifications' => $notifications]);
    }
    ...
    public function requestlist()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "MIC" AND request_status != 4 AND request_status != 5 AND deleted_at IS NULL AND requested_by = ?', [$usersession]);
        ...
        return view('mic/micrequest', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function requestlist2()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "MIC" AND deleted_at IS NULL AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        ...
        return view('mic/micrequest2', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    ...
    public function micaddrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            if($quantity == NULL){
                ...
                return redirect('micrequestadd')->with('message', 'error');
            }

            foreach ($quantity as $index => $data) {
                $insert = new MIC_Order_List_Model;

                $insert->mic_preriv_id = $request_id;
            ...
            $count_data = MIC_Order_List_Model::where('mic_preriv_id', $request_id)->count();

            $total_data = MIC_Order_List_Model::where('mic_preriv_id', $request_id)->value(DB::raw("SUM(quantity * unit_price)"));
            ...
            try {
                ...
                return redirect('micrequest')->with('message', 'failed');
            }

            return redirect('micrequest')->with('message', 'success');
        }
    }

    public function micdetails(Request $request)
    {
        ...
        if ($approve_order == NULL) {
            ...
            foreach ($approve_order as  $itemcheck) {
                ...
                $approve_item[] = DB::select('SELECT ItemDet FROM micppmp WHERE ppmpID = ?', [$item]);
            }
        }
        return view('mic/michistory', ['requesthistory' => $req_data], compact('order_data', 'item_data', 'approve_order', 'approve_item', 'user_data', 'requestor_data', 'notifications'));
    }

    public function micdetails2(Request $request)
    {
    ...
    public function micrequestview(Request $request)
    {
        ...
        return view('mic/micrequestview', ['requestview' => $req_data], compact('order_data', 'item_data', 'requestor_data', 'notifications'));
    }

    public function micrequestedit(Request $request)
    {
        ...
        return view('mic/micrequestedit', ['requestview' => $req_data], compact('order_data', 'item_data', 'notifications'));
    }

    public function miceditrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            return redirect('micrequest')->with('message', 'update');
        }
    }
    ...
    public function deleterequest(Request $request)
    {
        ...
        return redirect('micrequest');
    }

    public function mictrash()
    {
        $usersession = Session::get('user');
        $user = AdminModel::find($usersession);
        $notifications = $user->unreadNotifications;

        $data = DB::select('SELECT * FROM request WHERE department = "MIC" AND deleted_at IS NOT NULL AND requested_by = ?', [$usersession]);
        ...
        return view('mic/mictrash', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function micrestoreall()
    {
        $restore = RequestModel::onlyTrashed()->restore();

        return redirect('mictrash')->with('message', 'restored');
    }
    public function miceraseall()
    {
        $restore = RequestModel::onlyTrashed()->forceDelete();

        return redirect('mictrash')->with('message', 'erased');
    }
    public function micrestore(Request $request)
    {
    ...
    public function micerase(Request $request)
    {
