...
class GCSCController extends Controller
{
    public function gcsc()
    {
        ...
        $activeRequest = DB::select('SELECT * FROM request WHERE department = "GCSC" AND request_status != 4 AND request_status != 5 AND requested_by = ?', [$usersession]);
        $countActive = count($activeRequest);

        $closeRequest = DB::select('SELECT * FROM request WHERE department = "GCSC" AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        $countClosed = count($closeRequest);

        return view('gcsc/gcsc', ['notifications' => $notifications], compact('username', 'itemJson', 'countItem', 'countActive', 'countClosed'));
    }
    ...
    public function gcscproplan()
    {
        ...
        return view('gcsc/gcscproplan', ['proplandata' => $data], ['notifications' => $notifications]);
    }
    ...
    public function requestlist()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "GCSC" AND request_status != 4 AND request_status != 5 AND deleted_at IS NULL AND requested_by = ?', [$usersession]);
        ...
        return view('gcsc/gcscrequest', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function requestlist2()
    {
        ...
        $data = DB::select('SELECT * FROM request WHERE department = "GCSC" AND deleted_at IS NULL AND requested_by = ? AND request_status = 4 OR request_status = 5 ', [$usersession]);
        ...
        return view('gcsc/gcscrequest2', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    ...
    public function gcscaddrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            if($quantity == NULL){
                ...
                return redirect('gcscrequestadd')->with('message', 'error');
            }

            foreach ($quantity as $index => $data) {
                $insert = new GCSC_Order_List_Model;

                $insert->gcsc_preriv_id = $request_id;
            ...
            $count_data = GCSC_Order_List_Model::where('gcsc_preriv_id', $request_id)->count();

            $total_data = GCSC_Order_List_Model::where('gcsc_preriv_id', $request_id)->value(DB::raw("SUM(quantity * unit_price)"));
            ...
            try {
                ...
                return redirect('gcscrequest')->with('message', 'failed');
            }

            return redirect('gcscrequest')->with('message', 'success');
        }
    }

    public function gcscdetails(Request $request)
    {
        ...
        if ($approve_order == NULL) {
            ...
            foreach ($approve_order as  $itemcheck) {
                ...
                $approve_item[] = DB::select('SELECT ItemDet FROM gcscppmp WHERE ppmpID = ?', [$item]);
            }
        }
        return view('gcsc/gcschistory', ['requesthistory' => $req_data], compact('order_data', 'item_data', 'approve_order', 'approve_item', 'user_data', 'requestor_data', 'notifications'));
    }

    public function gcscdetails2(Request $request)
    {
    ...
    public function gcscrequestview(Request $request)
    {
        ...
        return view('gcsc/gcscrequestview', ['requestview' => $req_data], compact('order_data', 'item_data', 'requestor_data', 'notifications'));
    }

    public function gcscrequestedit(Request $request)
    {
        ...
        return view('gcsc/gcscrequestedit', ['requestview' => $req_data], compact('order_data', 'item_data', 'notifications'));
    }

    public function gcsceditrequest(Request $request)
    {
        ...
        if ($insert) {
            ...
            return redirect('gcscrequest')->with('message', 'update');
        }
    }
    ...
    public function deleterequest(Request $request)
    {
        ...
        return redirect('gcscrequest');
    }

    public function gcsctrash()
    {
        $usersession = Session::get('user');
        $user = AdminModel::find($usersession);
        $notifications = $user->unreadNotifications;

        $data = DB::select('SELECT * FROM request WHERE department = "GCSC" AND deleted_at IS NOT NULL AND requested_by = ?', [$usersession]);
        ...
        return view('gcsc/gcsctrash', compact('data', 'requestor_data'), ['notifications' => $notifications]);
    }
    public function gcscrestoreall()
    {
        $restore = RequestModel::onlyTrashed()->restore();

        return redirect('gcsctrash')->with('message', 'restored');;
    }
    public function gcsceraseall()
    {
        $restore = RequestModel::onlyTrashed()->forceDelete();

        return redirect('gcsctrash')->with('message', 'erased');;
    }
    public function gcscrestore(Request $request)
    {
    ...
    public function gcscerase(Request $request)
    {
